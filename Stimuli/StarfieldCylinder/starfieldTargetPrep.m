function output = starfieldTargetPrep(Parameters, ScreenData, StimSettings, NumSubframes)
...
P.targetSize  = Parameters(9,:);
P.N           = Parameters(10,:)*NumSubframes;
...
%         z = C(3,:);
        ...
        % Target
        ifi = ScreenData.ifi;
        target_x = eval(StimSettings(k).edit2{2});
        target_y = eval(StimSettings(k).edit3{2});
        target_z = eval(StimSettings(k).edit4{2});
        
        azimuth = atand(target_y/target_x);
        elevation = atand(-target_z/target_x);
        
        xx = oX + azimuth*pxPerDegree;
        yy = oY + tand(elevation)*elevationFactor;
        
        output(k).xymatrix{n} = [output(k).xymatrix{n} [xx; yy]];
        output(k).color{n} = [output(k).color{n} [0;0;0;255]];
        output(k).dotsize{n} = [output(k).dotsize{n} 32];
    end
